...
class ClusterStatsRotatorTest {
    ...
    @Test
    fun durationToMinutesForNavControl() {
        assertThat(MapsNavParser.durationToMinutes("15m")).isEqualTo(15)
        assertThat(MapsNavParser.durationToMinutes("1h 10m")).isEqualTo(70)
        assertThat(MapsNavParser.durationToMinutes("1 hr 5 min")).isEqualTo(65)
        assertThat(MapsNavParser.durationToMinutes(null)).isEqualTo(0)
    }

    @Test
    fun remainingMetersIgnoresNextTurnScale() {
        val bad = MapsNavSnapshot(
            nextTurnDistanceText = "200 m",
            nextTurnDistanceMeters = 200,
            remainingDistanceText = "200 m",
            timeToDestinationText = "15m",
            updatedAtMs = System.currentTimeMillis()
        )
        assertThat(bad.remainingDistanceMetersOrZero()).isEqualTo(0)
        val good = bad.copy(remainingDistanceText = "4.2 km")
        assertThat(good.remainingDistanceMetersOrZero()).isEqualTo(4200)
        assertThat(good.pictogramId()).isEqualTo(7) // no instruction → straight
        val left = good.copy(nextTurnInstruction = "Turn left onto Mount Road")
        assertThat(left.pictogramId()).isEqualTo(0)
    }
