...
data class MapsNavSnapshot(
    ...
    /** Pictogram byte for the native cluster arrow (right-side symbol). */
    fun pictogramId(): Int =
        ManeuverPictogram.fromInstruction(nextTurnInstruction)

    /** Remaining trip meters for nav control bytes 6–8 (never next-turn alone). */
    fun remainingDistanceMetersOrZero(): Int {
        val rem = remainingDistanceText?.let { MapsNavParser.distanceToMeters(it) }
        val next = nextTurnDistanceMeters
        if (rem == null) return 0
        // Guard: if remaining collapsed to next-turn scale, do not advertise it as dest left.
        if (next != null && rem <= next + 50 && rem < 500) return 0
        return rem
    }

    fun remainingTimeMinutesOrZero(): Int =
        MapsNavParser.durationToMinutes(timeToDestinationText)
...
object MapsNavParser {
    ...
    /** Parse `15m` / `1h 20m` / `1 hr 10 min` into whole minutes for nav control bytes 4–5. */
    fun durationToMinutes(raw: String?): Int {
        if (raw.isNullOrBlank()) return 0
        val text = asTravelDuration(raw) ?: raw.trim()
        val hours = Regex("""(\d+)\s*h""", RegexOption.IGNORE_CASE).find(text)
            ?.groupValues?.getOrNull(1)?.toIntOrNull() ?: 0
        val mins = Regex("""(\d+)\s*m""", RegexOption.IGNORE_CASE).find(text)
            ?.groupValues?.getOrNull(1)?.toIntOrNull()
        if (hours > 0 || mins != null) return (hours * 60) + (mins ?: 0)
